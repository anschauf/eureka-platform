...
import {
  DECLINE_ARTICLE_NOT_ENOUGH_REVIEWERS
} from './ButtonsNaming.js';
...
export const DeclineArticleNotEnoughReviewersButton = connect(
  mapStateToProps,
  mapDispatchToProps
)(props => {
  return (
      ...
      {web3Context => {
        return (
          <ActionButton
            ...
            dataTip={'notEnoughReviewers'}
            onClick={() => {
              declineArticleAndCloseSubmission(web3Context, props, () => {
                props.fetchingArticleData(props.article._id);
              });
            }}
            title={DECLINE_ARTICLE_NOT_ENOUGH_REVIEWERS.tooltip}
          >
            {DECLINE_ARTICLE_NOT_ENOUGH_REVIEWERS.label}
          </ActionButton>
        );
      }}
    </Web3Context.Consumer>
